...
<details>
Remember that there are essentially two conventions when it comes to NDC space. Coordinates are either defined in the range [-1, 1] or [0, 1]. The range [-1, 1] is used with most real-time graphics APIs, such as OpenGL or Direct3D. The RenderMan specifications define their space with range [0, 1]. You are entirely free to choose the convention you prefer. We will stick to the convention used by graphics API (using the range [-1, 1]) because it is essential to see these matrices being used in this context. 
</details>
